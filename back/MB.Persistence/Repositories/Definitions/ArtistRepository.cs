...
public class ArtistRepository(Context context) : BaseRepository<Artist>(context), IArtistRepository
{
    ...
    public async Task DeleteArtistRelations(int artistId)
    {
    ...
    public async Task RemoveArtistStylesAsync(Artist artist)
    {
        _context.RArtistStyle.RemoveRange(artist.ArtistStyles ?? []);
        await _context.SaveChangesAsync();
    }

    public async Task AddArtistStylesAsync(int artistId, IEnumerable<int> styleIds)
    {
